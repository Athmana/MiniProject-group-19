...
import 'dart:async';
import 'package:gowayanad/frontend/driver/driverequestscreen.dart';
...
class _DriverHomePageState extends State<DriverHomePage> {
  ...
  void _startListeningToEarnings() {
    final driverId = _auth.currentUser?.uid ?? 'anonymous_driver';
  ...
  Future<void> _updatePosition() async {
    try {
      // Use the centralized LocationService which handles permissions/Gps status
      ...
      if (mounted) {
        String errorMessage = "Location or Permission Error. Please check settings.";
        if (e is String) {
          errorMessage = e;
        } else if (e.toString().contains("denied")) {
           errorMessage = "Location Permission Denied. Please grant in settings.";
        } else if (e.toString().contains("disabled")) {
           errorMessage = "GPS is disabled. Please turn it ON.";
        }
  ...
  void _startListeningForRides() {
    debugPrint("DEBUG: Broadcast listening enabled...");
    // We now just let the StreamBuilder in the build method handle the UI
    // No need to auto-push a full screen anymore, giving more control to driver.
  }
          ...
          SafeArea(
            child: Column(
              children: [
                ...
                // --- Broadcast Requests Carousel ---
                if (_isOnline) _buildBroadcastRequestsSection(),
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
              color: (_isOnline ? Colors.green : AppColors.error).withValues(alpha: 0.2),
              blurRadius: 10,
  ...
  Widget _buildBroadcastRequestsSection() {
    return StreamBuilder<QuerySnapshot>(
      stream: _rideService.getBroadcastedRequests(),
      builder: (context, snapshot) {
        ...
        final docs = snapshot.data!.docs.where((doc) {
          ...
          if (driverType != null && driverType.isNotEmpty && rideType != null) {
            if (driverType.toLowerCase() != rideType.toLowerCase()) return false;
          }

          // Distance Check (Within 10km)
          ...
          children: [
            ...
            const Padding(
              ...
              child: Row(
                children: [
                  ...
                  Spacer(),
                  Text("Slide and tap to view", style: TextStyle(color: Colors.grey, fontSize: 10)),
                ],
              ),
            ),
            ...
            SizedBox(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  final ride = docs[index].data() as Map<String, dynamic>;
                  return _buildBroadcastCard(docs[index].id, ride);
                },
              ),
            ),
          ],
  ...
  Widget _buildBroadcastCard(String rideId, Map<String, dynamic> ride) {
    return Container(
      width: 280,
