...
import 'package:gowayanad/frontend/screens/waitingfordriverscreen.dart';
import 'package:geolocator/geolocator.dart';
import 'package:geocoding/geocoding.dart';
import 'package:gowayanad/others/utils/fare_calculator.dart';
import 'package:gowayanad/others/utils/design_system.dart';
...
class _RiderBookingScreenState extends State<RiderBookingScreen> {
  ...
  Future<void> _fetchLocation() async {
    try {
      ...
      List<Placemark> placemarks = await _geocodingService.placemarkFromCoordinates(
        position.latitude,
        position.longitude,
      );
  ...
  Future<void> _calculateFares(String destination) async {
    ...
    try {
      ...
      List<Location> locations = await _geocodingService.locationFromAddress(destination);
            ...
            CustomButton(
              ...
              onPressed: (_isLoadingLocation || _isCalculatingFare)
                  ...
                  : () async {
                      ...
                      if (mounted) setState(() => _isCalculatingFare = true);
                      try {
                        final String? rideId = await _rideService.requestRide(
